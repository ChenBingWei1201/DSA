...
SkipNode *createNode(long long key,
                     int level) { // create a SkipNode with key and level
...
SkipList *
createSkipList(int max_level) { // create an empty SkipList with its max level
  SkipList *list = (SkipList *)malloc(sizeof(SkipList));
  list->max_level = max_level;
  list->level = 0;
  list->header = createNode(
      LLONG_MAX,
      max_level); // the header is the node with key value = LLONG_MAX, which
                  // means the header won't disappear
...
void insertNode(SkipList *list,
                long long key) { // insert a node with key into skip list
  SkipNode
      *update[list->max_level + 1]; // store the previous node in each layer
...
void deleteNode(SkipList *list,
                long long key) { // delete a node with key from skip list
  ...
  if (current != NULL && current->key == key) {
    ...
    // delete nodes after `update[i]` when `update[i] -> next[i]` equals to
    // `current`
...
int main() {
  SkipList *list =
      createSkipList(64); // create an empty skip list with max layer = 64
  int M, t;
  long long k;
  scanf("%d", &M);
  for (int i = 0; i < M; i++) {
